...
impl<'a> OracleMap<'a> {
    ...
    pub fn get_account_info(&self, pubkey: &Pubkey) -> VortexDexResult<AccountInfo<'a>> {
        Ok(self
            .oracles
            .get(pubkey)
    ...
    pub fn get_price_data(&mut self, pubkey: &Pubkey) -> VortexDexResult<&OraclePriceData> {
    ...
    ) -> VortexDexResult<(&OraclePriceData, OracleValidity)> {
        if self.should_get_quote_asset_price_data(pubkey) {
    ...
    pub fn get_price_data_and_guard_rails(
        &mut self,
        pubkey: &Pubkey,
    ) -> VortexDexResult<(&OraclePriceData, &ValidityGuardRails)> {
        if self.should_get_quote_asset_price_data(pubkey) {
    ...
    pub fn validate_oracle_account_info<'c>(account_info: &'c AccountInfo<'a>) -> VortexDexResult {
