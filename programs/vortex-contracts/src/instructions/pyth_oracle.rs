...
use pyth_solana_receiver_sdk::{cpi::accounts::{PostUpdate, PostUpdateAtomic}, price_update::PriceUpdateV2, program::PythSolanaReceiver, PostMultiUpdatesAtomicParams, PostUpdateAtomicParams, PostUpdateParams};

use crate::{ids::{vortex_oracle_program, wormhole_program}, validate};
...
pub struct UpdatePythPullOraclePriceFeed<'info> {
    ...
    #[account(mut, seeds = [PTYH_PRICE_FEED_SEED_PREFIX, &feed_id], bump, owner = vortex_oracle_program::id())]
...
pub struct PostPythPullOracleUpdateAtomic<'info> {
    ...
    /// CHECK: This account's seeds are checked
    #[account(mut, owner = vortex_oracle_program::id(), seeds = [PTYH_PRICE_FEED_SEED_PREFIX, &feed_id], bump)]
    pub price_feed: AccountInfo<'info>,
}
